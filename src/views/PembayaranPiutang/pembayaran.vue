<template>
  <v-col cols="12" md="12" v-resize="onResize">
    ...
    <v-card color="grey lighten-3" class="pa-2 rounded-lg elevation-0">
      ...
      <ejs-grid
        :dataSource="ItemsPerkiraanPiutang"
        height="200"
        width="100%"
        gridLines="Both"
        :allowReordering="true"
...
export default {
  ...
  watch: {
    ...
    ItemsPembayaran(val) {
      this.ItemsPerkiraanPiutang = val;
    },

    title(ket) {
      // console.log('title',ket)
      if (ket == "Kosongkan") {
        this.ItemsPerkiraanPiutang = [];
        this.hapus_items = [];
      }
    },

    ItemsPerkiraanPiutang(val) {
      // console.log(val)
  ...
  methods: {
    ...
    customValidationFn(args) {
      ...
      // if (this.title == 'Ubah') {
      // if (parseFloat(args.value) <= parseFloat(this.totalBayar) && parseFloat(this.totalBayarPerkiraan) <= parseFloat(this.totalBayar)) {
      //   if (this.ItemsPerkiraanPiutang.length > 1) {
      //     let val = parseFloat(args.value) + parseFloat(this.totalBayarPerkiraan)
      //     if (val <= parseFloat(this.totalBayar)) {
      //       return true
      //     }
      //   } else {
      //     return true
    ...
    PilihPerkiraan() {
      this.ItemsPerkiraanPiutang = [...this.ItemsPerkiraanPiutang];

      if (this.ItemsPerkiraanPiutang.length > 0) {
        for (let index = 0; index < this.storeSelect.length; index++) {
          const data = {
            Kode: this.storeSelect[index].Kode,
            Nama: this.storeSelect[index].Nama,
            NoUrut: index == 0 ? index + 1 + parseInt(this.ItemsPerkiraanPiutang[this.ItemsPerkiraanPiutang.length - 1].NoUrut) : parseInt(this.ItemsPerkiraanPiutang[this.ItemsPerkiraanPiutang.length - 1].NoUrut) + 1,
            Jumlah: 0,
            Keterangan: ""
          }
          this.ItemsPerkiraanPiutang.push(data);
        }
      } else {
        for (let index = 0; index < this.storeSelect.length; index++) {
          ...
          element.Jumlah = 0;
          element.Keterangan = "";
          this.ItemsPerkiraanPiutang.push(element);
        }
      }
    ...
    onActionComplete(args) {
      ...
      if (args.requestType === "batchsave" || args.requestType === "save") {
        // let data = this.ItemsPerkiraanPiutang
        // this.ItemsPerkiraanPiutang = data.map(item => {
        //   return {
        //     id:item.id,
