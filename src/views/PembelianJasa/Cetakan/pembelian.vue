                                    ...
                                    <table class="isi">
                                        ...
                                        <tbody>
                                            <tr v-for="(item,index) in dataReport.items" :key="index">
                                                <td>{{index+1}}</td>
                                                <td>{{item.Perkiraan}} {{item.perkiraan.Nama}}</td>
                                                <td>{{item.Keterangan}}</td>
                                                <td class="nominal">{{item.Jumlah}}</td>
                                                <td class="nominal">{{(item.Harga - parseFloat(item.Diskon)).toLocaleString(undefined)}}</td>
                                                <td class="nominal">{{(item.SubTotal - parseFloat(item.SubDiskon)).toLocaleString(undefined)}}</td>
                                            </tr>
                                        </tbody>
                                    </table>
                                </div>
                                ...
                                <div class="footer">
                                    ...
                                    <table style="width:100%;">
                                        <tr>
                                            ...
                                            <td style="text-align:right; width:20%;">{{parseFloat(dataReport.DPP).toLocaleString(undefined)}}</td>
                                        </tr>
                                        <tr>
                                            <!-- <td>&nbsp;</td>
                                            <td>&nbsp;</td> -->
                                            <td style="width:10%; text-align:right;">PPn :</td>
                                            <td style="text-align:right; width:20%;">{{parseFloat(dataReport.PPn).toLocaleString(undefined)}}</td>
                                        </tr>
                                        <tr>
                                            ...
                                            <td style="text-align:right; width:20%;">{{parseFloat(dataReport.TotalBayar).toLocaleString(undefined)}}0</td>
                                        </tr>
                                    </table>
                                    <!-- ttd -->
                                    <table style="width:100%; margin-top:10px; border-collapse: collapse;">
                                        <tr>
                                            <td style="padding-left:5px; height:50px; vertical-align:text-top; font-size:12px; border: 1px solid black; font-weight:none;">Admin Pembelian,</td>
                                            <td style="padding-left:5px; height:50px; vertical-align:text-top; font-size:12px; border: 1px solid black; font-weight:none;">Di Setujui Oleh,</td>
...
export default {
    ...
    created(){
        ...
        this.dataReport.Terbilang = angkaTerbilang(parseFloat(this.dataReport.TotalBayar)).toUpperCase() + ' RUPIAH'
        this.dataReport.KodeNota = this.$route.params.data.KodeNota
        this.dataReport.Keterangan = this.$route.params.data.Keterangan
        // this.dataReport.NoPR = this.$route.params.data.items.find( ({detail_po}) => detail_po.NoPR !== null).detail_po.NoPR
    ...
    methods:{
        countCetak(id,token){
            api.put("countCetakan/"+id+'?token='+token,{modul:"BELI"})
            .then((res)=>{
                console.log(res)
            })
            .catch((err) => {
