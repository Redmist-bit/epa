<template>
              ...
              <v-card>
                <v-container>
                  <v-row>
                            ...
                            <v-card>
                              ...
                              <v-card flat>
                                <v-col cols="12" md="12">
                                  <v-card>
                                    <ejs-grid
                                    :dataSource="Perkiraan" height="200" width="100%" gridLines='Both'
                    ...
                    <v-col cols='12' sm='6' md='6'>
                      <v-text-field label="Keterangan" v-model="detail.Keterangan">
                      </v-text-field>
                    </v-col>
                    <v-col cols='12' sm='6' md='6'>
    ...
    <v-card color="grey lighten-3" class="pa-2 rounded-lg elevation-0">
      ...
      <ejs-grid 
        id="Grid"
        :dataSource="listDetail" height='200' width='100%' gridLines='Both'
        :allowReordering = true
      ...
      >
        <e-columns>   
          <e-column field="Commands" :visible='mobile' headerText="Aksi" width="110" textAlign='Center' :commands="commands" :lockColumn='true'>
          </e-column>
...
import { GridPlugin, Toolbar, Page, Aggregate, Resize, Filter, Sort, Edit, CommandColumn, Reorder } from "@syncfusion/ej2-vue-grids";
Vue.use(GridPlugin)
export default {
  props:{
    title: String,
  ...
  data(){
    return {
      ...
      dialogUang:false,
      itemsDepartemen:['PROCESSING','FINANCE ACCOUNTING','HRGA','SCM','SERVICE','IT MARCOMM','HSE','AUDIT','ID'],
      itemsLokasi:['HO','BERAU','PALEMBANG','SUNGAI DANAU','TABANG','MOROWALI','PALU'],
      itemsJenisWorkOrder:[
        'Konstruksi Gedung',
        'Konstruksi Jalan',
      ...
      dialogNoWO:false,
			pageSettings: {pageSize: 100, pageSizes :['5','10','15','20','50','100']},
      detail:{
        Perkiraan : '',
        NomorWO : '',
        Lokasi : '',
  ...
  provide:{
    grid: [Page, Toolbar, Aggregate, Resize, Filter, Sort, Edit, CommandColumn, Reorder]
  },
  watch:{
    windowSize(){
  ...
  methods:{
    ...
    commandClick: function(args) {
      if (args.target.classList.contains("Edit")) {
        ...
        this.listDetail.splice(this.listDetail.findIndex(a => a == args.rowData),1)
        document.getElementById("Grid").ej2_instances[0].refresh();
      }
    },
    Jumlah(){
      this.detail.Jumlah = parseFloat(this.detail.JumlahAsing) * parseFloat(this.detail.Kurs)
    },
    getMataUang(){
    ...
    Done(){
      if (this.listDetail.length == 0) {
        ...
        this.detail.NoUrut = this.listDetail.sort((a,b) => parseInt(b.NoUrut) - parseInt(a.NoUrut))[0].NoUrut + 1
        this.listDetail.push(this.detail)
      }
      this.listDetail = [...this.listDetail]
      document.getElementById("Grid").ej2_instances[0].refresh();
    ...
    dataStateChange: function(state) {
      if (state.action.requestType === 'paging') {
        ...
        if (state.search != undefined) {
          api.get('payment-voucher-wo?token='+this.token+'&search='+state.search[0].key+'&skip='+state.skip+'&take='+state.take).then(
            res => {
      ...
      if (state.action.requestType === "searching") {
        ...
        api.get('payment-voucher-wo?token='+this.token+'&search='+state.action.searchString+'&skip='+0+'&take='+100).then(
            res => {
                this.NomorWO = res.data
            }
        )
    ...
    getDataWo(skip,take){
      api.get('payment-voucher-wo?token='+this.token+'&skip='+skip+'&take='+take).then(
        res => {
          this.NomorWO = res.data
        },
        err => {
