		...
		<loading :active.sync="isLoading" :is-full-page="fullPage"></loading>
			...
			<template v-slot:default="dialog">
					...
					<v-card-actions class="justify-end">
							...
							<v-btn
							text
							@click="getDataPembelian(tanggal_awal,tanggal_akhir)"
							>Ok</v-btn>
					</v-card-actions>
					</v-card>
			</template>
			...
			<div :class="mobile ? 'pa-3' : ''">
				<v-card tile outlined :class="mobile ? 'elevation-2 pa-2' : ''" :style="mobile ? '' : 'margin-bottom: 90px;'">
					<v-toolbar flat v-show="mobile == false">
						<v-toolbar-title class="text-h5">
							<strong>{{$t('BeliJasa.MainTitle')}}</strong>
						</v-toolbar-title>
						<v-spacer></v-spacer>
						<v-btn
							icon
								...
								<v-card outlined class="mt-6">
										...
										<v-row>
												...
												>
														...
														<v-dialog v-model="dialogPilihsupplier" scrollable max-width="800px" persistent>
																<template v-slot:activator="{on, attrs}">
																<v-btn
																		dark
																		small
																...
																<v-toolbar
																		...
																		class="elevation-1">
																		<v-card-title>
																		<span class="headline">{{ formTitlePilihsupplier }}</span>
																		</v-card-title>
																		<v-spacer></v-spacer>
																		<v-btn
																		dark
																...
																<v-card flat><!-- Tabel Data supplier -->
																		<v-col cols="12" md="12">
																		<v-card>
																				<ejs-grid
												...
												>
														<template v-slot:append>
														<v-dialog v-model="dialogPilihBillFrom" scrollable max-width="800px" persistent>
																<template v-slot:activator="{on, attrs}">
																<v-btn
																		dark
																		small
																...
																<v-toolbar
																		...
																		class="elevation-1">
																		<v-card-title>
																		<span class="headline">{{ formTitlePilihBillFrom }}</span>
																		</v-card-title>
																		<v-spacer></v-spacer>              
																		<v-btn
																		dark
												...
												>
														<template v-slot:append>
														<v-dialog v-model="dialogPilihSellFrom" scrollable max-width="800px" persistent>
																<template v-slot:activator="{on, attrs}">
																<v-btn
																		dark
																		small
																...
																<v-toolbar
																		...
																		class="elevation-1">
																		<v-card-title>
																		<span class="headline">{{ formTitlePilihSellFrom }}</span>
																		</v-card-title>
																		<v-spacer></v-spacer>             
																		<v-btn
																		dark
												...
												>
														<template v-slot:append>
														<v-dialog v-model="dialogno_po" scrollable max-width="900px" persistent>
																<template v-slot:activator="{on, attrs}">
																<v-btn
																		dark
																		small
																...
																>
																<v-card-title>
																		<span class="headline">{{ formTitleno_po }}</span>
																</v-card-title>
																<v-spacer></v-spacer>             
																		<v-btn
																		dark
																...
																<v-card flat>
																		...
																		<v-card>
																				<ejs-grid
																				:dataSource="no_po" height="200" width="100%" gridLines='Both'
																				:allowReordering = true
																				:allowSorting='true'
																				:allowResizing='true'
																				:allowPaging='true'
												...
												<v-col cols="12" sm="6" md="4">
														<v-text-field dense
														readonly
														v-model="editedItem.NoWorkOrder"
														label="No. Work Order"></v-text-field>
												</v-col>
												...
												<v-card class="elevation-5" outlined>
														...
														<v-tab>ITEMS</v-tab>
																...
																<ItemsPembelian ref='ItemsPb' v-on:hapus_item="hps_items($event)" v-on:itemsPb="items($event)" v-bind:itemsPerkiraan="dataPerkiraan" v-bind:itemsPekerjaan="dataPekerjaan" v-bind:ItemsPembelian='ItemsPembelian' v-bind:itemspo="itemspo" :title="title"/>
																</v-tab-item>
				...
				<div id="app">
						<ejs-grid
							:dataSource="DataPembelian" :height="mobile ? '43vh' : 'auto'" width="100%" gridLines='Both'
							:allowReordering = true
						...
						<e-columns>
								...
								:edit='numericParams' editType= 'numericedit'
								:customAttributes="columnEdit"
								:allowEditing="actionEdit"
								width=180
								></e-column>
...
<script>
    ...
    import ItemsPembelian from '@/views/Pembelian/jasa'
    ...
    export default {
        ...
        return {
            ...
            ppn_persen: {locale:"id-ID", prefix: "ppn", suffix:"%", length:3, precision:0,},
            ppn: {locale:"id-ID", length:12, precision:2,},
            total_bayar: {locale:"id-ID", prefix: "Total Bayar", suffix:"", length:12, precision:2,},
            ItemsPembelian : [],
            editedIndex: -1,
            defaultItem: {
                KodeNota: "",
            ...
            pageSettings: {pageSize: 5, pageSizes :['5','10','15','20','50','100']},
        ...
        mounted(){
            if (this.action.some(a => a == 'R')) {
                ...
                this.getDataPembelian(JSON.parse(localStorage.getItem('Periode'))[0].TglAwal,JSON.parse(localStorage.getItem('Periode'))[0].TglAkhir)
            }
            ...
            if (this.action.some(a => a == 'D')) {
                this.commands.push(
                { title:"Batal/Hapus", buttonOption: { cssClass: 'e-flat Delete', iconCss: 'e-delete e-icons' } },
                )
            }
        ...
        computed: {
        formTitlePembelian(){
            return this.editedIndex === -1 ? "Tambah Pembelian Jasa" : "Ubah Pembelian Jasa";
        },
        formTitleno_po(){
        ...
        methods: {
			...
			async getDataPrint(){
				...
				await api.get('report-pembelian/'+this.editedItem.id+'?token='+this.token).then(
					res => {
						// console.log(res)
						this.$router.push({
						name:'cetakBeliJasa',
        ...
        Batalin(val) {
            api.put("/batal-beli/"+this.editedItem.id+'?token='+this.token,{
            keterangan:val
            })
            .then((res)=>{
            console.log(res)
        ...
        items(data){
            ...
            this.editedItem.Total = data.reduce((a,b) => a + parseFloat(b.Jumlah*b.Harga), 0)
    ...
    if (this.editedItem.PaymentTerm === '') {
        ...
        async savePilihsupplier(){
            await api.get('pembelian-po-jasa?token='+this.token+'&supplier='+this.kodeSupplier)
            .then((res)=>{
        ...
        async savePilihno_po(){
            this.title = "Tambah"
            await api.get('items-po-jasa/'+this.id_po+'?token='+this.token).then(
            res=>{
                this.itemspo = res.data.data.filter(v => parseFloat(v.Terpenuhi) < v.Jumlah).map(v => {
        ...
        TambahData(){
            ...
            // });
            this.editedItem.items = this.ItemsPembelian.map(j => {
                j.Perkiraan = this.dataPerkiraan.find(p => p.Nama == j.Perkiraan).Kode
                j.JenisPekerjaan = this.dataPekerjaan.find(p => p.Nama == j.JenisPekerjaan).Kode
                return j
            })
            this.isLoading = true
            api.post('/pembelian-jasa?token='+this.token, this.editedItem)
            .then((res) => {
        ...
        UpdateData(){
            ...
            this.editedItem.hapus_items = this.hapus_items.map(f => {
                f.JenisPekerjaan = this.dataPekerjaan.find(p => p.Nama == f.JenisPekerjaan).Kode
                return f
            })
            this.editedItem.items = this.ItemsPembelian.filter(v => v.KodeNota != undefined).map(j => {
                j.Perkiraan = this.dataPerkiraan.find(p => p.Nama == j.Perkiraan).Kode
                j.JenisPekerjaan = this.dataPekerjaan.find(p => p.Nama == j.JenisPekerjaan).Kode
                return j
            })
            this.editedItem.new_items = this.ItemsPembelian.filter(v => v.KodeNota == undefined).map(j => {
                j.Perkiraan = this.dataPerkiraan.find(p => p.Nama == j.Perkiraan).Kode
                j.JenisPekerjaan = this.dataPekerjaan.find(p => p.Nama == j.JenisPekerjaan).Kode
                return j
            })
            api.put('/pembelian-jasa/' + this.editedItem.id +'?token='+this.token, this.editedItem)
            .then((res)=>{
                console.log(res)
                this.hapus_items = []
                this.closePembelian()
        ...
        commandClick: function(args) {
          ...
          console.log('disni')
                ...
                api.get('pembelian-jasa/'+args.rowData.id+'?token='+this.token).then(
                    res=>{
                    ...
                    this.ItemsPembelian = res.data.data.items_jasa.map(v => {
                        v.DiskonRp = v.Diskon
                        v.Diskon = v.Diskon1
                        v.SubTotal = v.Jumlah * (parseFloat(v.Harga) - (parseFloat(v.Harga) * parseFloat(v.Diskon) / 100))
                        v.Perkiraan = v.perkiraan.Nama
                        v.JenisPekerjaan = v.pekerjaan.Nama
						v.maxUpdateJumlah = parseInt(v.detail_po.Jumlah) - parseInt(v.detail_po.Terpenuhi) + v.Jumlah
                        return v
                    })
            ...
            if(args.target.classList.contains("custombutton")) {
                ...
                if (args.rowData.Status == null) {
                    ...
                    if (r == true) {
                    api.delete('/pembelians/'+args.rowData.id+'?token='+this.token)
                    .then((res)=> {
                        console.log(res)
                        this.getDataPembelian(this.tanggal_awal,this.tanggal_akhir)
                    })
        ...
        actioncomplete(args) {
            if (args.requestType === "batchsave" || args.requestType === "save") {
                    ...
                    //     let arrUpdate = [
                    //     'PPnFaktur','CekFisikInv'
                    //     ]
                    //     if (args.previousData[key] != args.data[key] && arrUpdate.includes(key)) {
                    //         // console.log(key.includes('Tanggal'))
                    //         var conf = confirm(`ganti data ${key} lama ${args.previousData[key]} menjadi ${key} baru ${args.data[key]}`)
                    ...
                    //     }
                    // }
                    var conf = confirm(`Simpan Perubahan?`)
                    // console.log(`ganti data ${key} lama ${args.previousData[key]} menjadi ${key} baru ${args.data[key]}`)
                    // args.endEdit()
                    if (conf == true) {
                        this.UpdateDataGrid(args.data.id,args.data)
        ...
        UpdateDataGrid(id,data){
            this.isLoading = true
            api.put('grid-beli/'+id+'?token='+this.token,data)
            .then(() => {
                this.isLoading = false
            })
            .catch((err) => {
        ...
        getDataPembelian(from,to){
            ...
            this.isLoading = true
            api.get('/pembelian-jasa/'+from+'/'+to+'/?token='+this.token).then(
            res=>{
                this.isLoading = false
