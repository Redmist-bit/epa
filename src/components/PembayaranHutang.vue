      ...
      <template v-slot:default="dialog">
        <v-card>
          ...
          <v-card-actions class="justify-end">
            ...
            <v-btn
              text
              @click="getDataPembayaranHutang(tanggal_awal, tanggal_akhir)"
              >Ok</v-btn
            >
          </v-card-actions>
        </v-card>
      ...
      >
        <v-toolbar flat v-show="mobile == false">
          <v-toolbar-title class="text-h5">
            <strong>{{ $t("Hutang.MainTitle") }}</strong>
          </v-toolbar-title>
          <v-spacer></v-spacer>
          <v-btn
            icon
        ...
        >
          ...
          <v-dialog
            v-model="dialog_pembayaran_hutang"
            max-width="1100px"
            scrollable
            persistent
          >
            ...
            <v-card class="rounded-lg">
              <v-toolbar dark flat color="blue darken-4">
                <v-card-title>
                  <span class="headline">{{ formTitlePembayaranHutang }}</span>
                </v-card-title>
                <v-spacer></v-spacer>
                <v-btn dark text fab small @click="closePembayaranHutang()">
                    ...
                    <v-row>
                      ...
                      <v-col cols="12" sm="6" md="4">
                        <v-text-field
                          dense
                          readonly
                          v-model="editedItem.Supplier.Nama"
                          label="Supplier"
                        >
                          <template v-slot:append>
                            <v-dialog
                              v-model="dialog_pilih_supplier"
                              scrollable
                              max-width="900px"
                              persistent
                            >
                                  ...
                                  <v-col cols="12" md="12">
                                    <v-card>
                                      <ejs-grid
                                        ...
                                        width="100%"
                                        gridLines="Both"
                                        :pageSettings="pageSettingsSupplier"
                        ...
                        <v-card class="elevation-5" outlined>
                          ...
                          >
                            ...
                            <v-tab-item :eager="true">
                              <keep-alive>
                                <PEMBELIAN
                                  v-on:ItemsBELI="ItemsBELI($event)"
                                  v-on:hapus_item="hps_items($event)"
                                  :data_faktur="data_faktur_all"
                                  :ItemsPembelian="items_pembelian"
                                  :title="title"
                                />
                              </keep-alive>
                            </v-tab-item>
        ...
        <!-- table Pembayaran Hutang -->
        <div id="app">
          <ejs-grid
            :height="mobile ? '43vh' : 'auto'"
            width="100%"
            gridLines="Both"
            :dataSource="DataPembayaranHutang"
            :allowReordering="true"
      ...
      <template v-slot:default="dialog">
        <v-card>
          <v-toolbar color="primary" dark>
            <v-toolbar-title> Batal Pembayaran Hutang </v-toolbar-title>
          </v-toolbar>
          <v-card-text>
            <v-container>
              <v-text-field
      ...
      <v-card tile>
        ...
        <v-card-text>
          <v-sheet>
            ...
            <strong class="text-h5"><b>Pembelian</b></strong>
...
import PEMBELIAN from "../views/PembayaranHutang/pembelian.vue";
import PEMBAYARAN from "../views/PembayaranHutang/pembayaran.vue";
import {
  GridPlugin,
  Page,
  Aggregate,
...
export default {
  ...
  components: { PEMBELIAN, PEMBAYARAN, Loading },
  data() {
    return {
      alert: false,
      pesan: "",
      ...
      dialog_batal: false,
      dialog_periode: true,
      dialog_pilih_supplier: false,
      dialog_pembayaran_hutang: false,
      ...
      pageSettingsSupplier: { pageSize: 100, pageSizes: ["20", "50", "100"] },
  ...
  mounted() {
    ...
    this.getDataSupplier(0, 100, undefined, undefined);
  ...
  watch: {
    ...
    dialog_pembayaran_hutang(val) {
      val || this.closePembayaranHutang();
    },

    dialog_pilih_supplier(val) {
      val || this.close_pilih_supplier();
    },
  },

  computed: {
    formTitlePembayaranHutang() {
      return this.editedIndex === -1
        ? "Tambah Pembayaran Hutang"
        : "Ubah Pembayaran Hutang";
    },
  ...
  methods: {
    ...
    async getDataPrint() {
      ...
      console.log(rute);
      await api
        .get("report-hutang/" + this.editedItem.id + "?token=" + this.token)
        .then(
          (res) => {
            // console.log(res)
    ...
    Batalin(val) {
      api
        .put("/batal-hutang/" + this.editedItem.id + "?token=" + this.token, {
          keterangan: val,
        })
        .then((res) => {
          console.log(res);
          this.getDataPembayaranHutang(this.tanggal_awal, this.tanggal_akhir);
        })
        .catch((err) => {
          console.log(err);
        });
    ...
    getDataPembayaranHutang(from, to) {
      this.isLoading = true;
      api.get("/hutang/" + from + "/" + to + "?token=" + this.token).then(
        (res) => {
          this.isLoading = false;
          this.dialog_periode = false;
          this.DataPembayaranHutang = res.data.data;
        },
        (err) => {
          console.log(err);
          this.isLoading = false;
    ...
    getDataSupplier(skip, take, sort, filter) {
      // api.get("suppliers?token="+this.token).then(
      //   res => {
      //     this.data_supplier = res.data
      //   },
      //   err => {
      //     console.log(err)
      //   }
    ...
    dataStateChange: function (state) {
      ...
      if (
        ...
        this.getDataSupplier(state.skip, state.take, this.sorted);
        this.query = null;
        this.filter = null;
      }
      ...
      if (
        ...
        this.getDataSupplier(state.skip, state.take, undefined, this.filter);
        this.$refs.ggs.clearFiltering();
      }
    ...
    getDataMataUang() {
      api.get("mataUangs?token=" + this.token).then(
        (res) => {
          ...
          // this.editedItem.kurs = 1.00
          // this.defaultItem.kurs = 1.00
        },
        (err) => {
          console.log(err);
        }
    ...
    close_pilih_supplier() {
      this.dialog_pilih_supplier = false;
    },

    async savePilihSupplier() {
      this.title = "Tambah";
      await api
        .post("hutang-pembelian?token=" + this.token, {
          supplier: this.Namasupplier.Kode,
        })
        .then((res) => {
          // console.log(res)
          this.items_pembelian = res.data.data.map((d) => {
      ...
      this.close_pilih_supplier();
    },

    rowSelectedSupplier: function (args) {
      this.Namasupplier = args.data;
      this.editedItem.Supplier = args.data;
    },

    ItemsBELI(data) {
      this.DataBELI = data;
      this.editedItem.Total = this.DataBELI.reduce(
    ...
    closePembayaranHutang() {
      this.$nextTick(() => {
        ...
        this.dialog_pembayaran_hutang = false;
      });
    },

    TambahData() {
      this.isLoading = true;
      this.editedItem.pembelian = this.DataBELI;
      ...
      if (TotalBayar == parseFloat(this.editedItem.Total)) {
        this.editedItem.Supplier = this.editedItem.Supplier.Kode;
        this.editedItem.MataUang = this.mata_uang.find(
          (m) => m.Nama == this.editedItem.MataUang
        ).Kode;
        api
          .post("/hutang?token=" + this.token, this.editedItem)
          .then(() => {
            this.isLoading = false;
            this.closePembayaranHutang();
            this.getDataPembayaranHutang(this.tanggal_awal, this.tanggal_akhir);
          })
          .catch((err) => {
            console.log(err);
    ...
    UpdateData() {
      ...
      if (TotalBayar == parseFloat(this.editedItem.Total)) {
        ...
        api
          .put(
            "/hutang/" + this.editedItem.id + "?token=" + this.token,
            this.editedItem
          )
          .then(() => {
            this.isLoading = false;
    ...
    commandClick: function (args) {
      if (args.target.classList.contains("Details")) {
        console.log("disni");
        this.isLoading = true;
        api.get("hutang/" + args.rowData.id + "?token=" + this.token).then(
          (res) => {
            ...
            this.editedItem.Supplier = res.data.data.supplier;
            ...
            this.items_pembelian = res.data.data.itemspembelian.map((p) => {
              p.TotalBayar = p.faktur.TotalBayar;
              p.SisaBayar = p.faktur.SisaBayar;
      ...
      if (args.target.classList.contains("Delete")) {
        if (args.rowData.status == null) {
          ...
          if (r == true) {
            api
              .delete("/hutang/" + args.rowData.id + "?token=" + this.token)
              .then(() => {
                this.getDataPembayaranHutang(
                  this.tanggal_awal,
                  this.tanggal_akhir
        ...
        this.editedIndex = 1;
        this.title = "Ubah";
        this.dialog_pembayaran_hutang = true;
      }
    },
  },
};
