<template>
    <div v-resize="onResize">
        ...
        <v-dialog v-model="dialog_periode" transition="dialog-top-transition" max-width="400">
            <template v-slot:default="dialog">
                <v-card>
                <v-toolbar
                    color="primary"
                    dark
                >Choose Date</v-toolbar>
                <v-card-text>
                    <v-container>
                    ...
                    <v-combobox :items="items_periode" v-on:change="change_date" label="Periode"></v-combobox>
                    </v-container>
                </v-card-text>
        ...
        <div :class="mobile ? 'pa-3' : ''">
			<v-card tile outlined :class="mobile ? 'elevation-2 pa-2' : ''" :style="mobile ? '' : 'margin-bottom: 90px;'">
        <v-toolbar flat v-show="mobile == false">
          ...
          >
                ...
                <v-dialog v-model="dialog" scrollable persistent max-width='1200px'>
                    ...
                    <v-card>
                                ...
                                <v-col>
                                    <v-row>
                                        <v-col cols='12' sm='6' md='4'>
                                            <v-text-field dense v-model="editedItem.KodeNota" :disabled='true' label='Kode Nota'></v-text-field>
                                        </v-col>
                                        <v-col cols='12' sm='6' md='4'>
                                            <v-menu
                                            ...
                                            >
                                                <template v-slot:activator="{ on, attrs }">
                                                    <v-text-field dense v-model="editedItem.TanggalKirim" label="TanggalKirim" prepend-icon="mdi-calendar" readonly v-bind="attrs" v-on="on"></v-text-field>
                                                </template>
                                                <v-date-picker
                                                locale="id"
                                        ...
                                        <v-col cols='12' sm='6' md='4'>
                                            ...
                                            >
                                                <template v-slot:activator='{on,attrs}'>
                                                    <v-text-field dense v-model="editedItem.Tanggal" label="Tanggal" prepend-icon="mdi-calendar" readonly v-bind="attrs" v-on="on"></v-text-field>
                                                </template>
                                                <v-date-picker locale='id' :min="HariIni" v-model="editedItem.Tanggal" @input="menuTanggal = false"></v-date-picker>
                                            </v-menu>
                                        </v-col>
                                        <v-col cols='12' sm='6' md='4'>
                                            <v-text-field dense v-model="editedItem.Referensi" label='Referensi'></v-text-field>
                                        </v-col>
                                        <v-col cols="12" sm="6" md="4">
                                            <v-text-field dense
                                                ...
                                                <v-dialog v-model="dialogsupplier" max-width="800px" persistent>
                                                    <template v-slot:activator="{on, attrs}">
                                                    <v-btn
                                                        dark
                                                        small
                                                    ...
                                                    <v-col cols="12" md="12">
                                                        ...
                                                        <ejs-grid
                                                        id="GridSupplier"
                                                            :dataSource="supplier" height="200" width="100%" gridLines='Both'
                                        ...
                                        <v-col cols="12" sm="6" md="4">
                                            <v-text-field dense
                                                ...
                                                label="BillFrom">
                                                <template v-slot:append>
                                                <v-dialog v-model="dialogBillFrom" max-width="800px" persistent>
                                                    <template v-slot:activator="{on, attrs}">
                                                    <v-btn
                                                        dark
                                                        :disabled='lockedit'
                                        ...
                                        <v-col cols="12" sm="6" md="4">
                                            <v-text-field dense
                                                ...
                                                label="SellFrom">
                                                <template v-slot:append>
                                                <v-dialog v-model="dialogSellFrom" max-width="800px" persistent>
                                                    <template v-slot:activator="{on, attrs}">
                                                    <v-btn
                                                        dark
                                                        small
                                        ...
                                        <v-col cols="12" sm="6" md="4">
                                            <v-text-field
                                                ...
                                                <v-dialog v-model="dialogWorkOrder"  max-width="800px" persistent>
                                                    <template v-slot:activator="{on, attrs}">
                                                    <v-btn
                                                        dark
                                                        small
                                                    ...
                                                    <v-toolbar
                                                        ...
                                                        class="elevation-1">
                                                        <v-card-title>
                                                        <span class="headline">Pilih Work Order</span>
                                                        </v-card-title>
                                                        <v-spacer></v-spacer>
                                                    ...
                                                    <v-col cols="12" md="12">
                                                        <v-card>
                                                        <ejs-grid
                                                            :dataSource="WorkOrder" height="200" width="100%" gridLines='Both'
                                                            :allowReordering = true
                                        ...
                                        <v-col cols="12" md="4" sm="6">
                                            <v-text-field dense v-model="editedItem.MataUang" readonly label="Mata Uang">
                                                <template v-slot:append>
                                                <v-dialog v-model="dialogMataUang" scrollable max-width="800px" persistent>
                                                    <template v-slot:activator="{on, attrs}">
                                                        <v-btn
                                                            dark
                                                            small
                                                    ...
                                                    <v-card flat><!-- Tabel Data WorkOrder -->
                                                        <v-col cols="12" md="12">
                                                        <v-card>
                                                            <ejs-grid
                                                            :dataSource="MataUang" height="200" width="100%" gridLines='Both'
                                                            :pageSettings='pageSettings'
                                            ...
                                            <v-card class="elevation-5" outlined>
                                                ...
                                                <v-tab>Account</v-tab>
                                                    <v-tab-item :eager="true">
                                                        <ItemsPoJasa v-on:hapus_item='hps_items($event)' v-on:dataPekerjaan='pekerjaan($event)' v-bind:title="title" v-on:dataPerkiraan='perkiraan($event)' v-on:items='itemsPoJasa($event)' v-bind:itemsJasa='itemJasa' v-bind:loadRwl="WorkLis"/>
                                                    </v-tab-item>
                                                    <v-divider class="mx-2" color= "" inset vertical></v-divider>
                                                </v-tabs>
                                            </v-card>
                                        </v-col>
                        ...
                        <v-card-actions>
                            ...
                            color="blue darken-4"
                            @click="save">
                            <v-icon class="mr-1">mdi-content-save</v-icon>{{$t('All.BtnSimpan')}}
                            </v-btn>
                        </v-card-actions>
                    </v-card>
                </v-dialog>
            ...
            <div id="app">
                <ejs-grid
                    :dataSource="data" :height="mobile ? '43vh' : 'auto'" width="100%" gridLines='Both'
                    :allowReordering = true
                ...
                >
                    <e-columns>
                        <e-column field="Commands" :visible='mobile' headerText="Aksi" width="115" :commands="commands" textAlign='center'>
                        </e-column>
                        <e-column headerText='Aksi' :visible='!mobile' width=150 textAlign='center' :commands='commandsMobile'></e-column>
        ...
        <v-dialog v-model="dialog_batal" transition="dialog-bottom-transition" max-width="400">
            <template v-slot:default="dialog">
                <v-card>
                <v-toolbar
                color="primary"
                dark
                >Batal PO Jasa</v-toolbar>
                <v-card-text>
                <v-container>
                <v-text-field
                    label="Alasan :"
                ...
                <v-btn
                text
                @click="Batalin(keterangan_batal)"
                >Ok</v-btn>
                </v-card-actions>
                </v-card>
            </template>
        </v-dialog>
        <v-dialog v-model="dialogEditSuplier" transition="dialog-bottom-transition" max-width="400">
            <template v-slot:default="dialog">
                <v-card>
                <v-toolbar
                color="primary"
                dark
                >Ganti Supplier PO</v-toolbar>
                <v-card-text>
                <v-container>
                    <v-text-field dense
                    v-model="editedItem.Supplier"
        ...
        <v-dialog v-model="dialog_print" transition="dialog-top-transition" max-width="400">
            <template v-slot:default="dialog">
                <v-card>
                <v-toolbar
                    color="primary"
                    dark
                >Choose Print Design</v-toolbar>

                <v-card-text>
                    <v-container>
                    <v-combobox :items="printOutItems" v-on:change="change_print" label="Print"></v-combobox>
                    </v-container>
                </v-card-text>
        ...
        <v-snackbar v-model="alert" rounded="lg" top color="error" :value="true" :timeout="-1">
            <span><v-icon left>mdi-alert-circle</v-icon>{{ pesan }} <v-icon @click="alert = false" style="float: right;">mdi-close</v-icon></span>
        </v-snackbar>
        <v-dialog
            ...
            <v-card-text>
              <v-sheet>
                ...
                <v-row no-gutters class="ml-n3 mr-n3">
                  <ItemsPoJasa v-on:hapus_item='hps_items($event)' v-on:dataPekerjaan='pekerjaan($event)' v-on:dataPerkiraan='perkiraan($event)' v-on:items='itemsPoJasa($event)' v-bind:itemsJasa='itemJasa' v-bind:loadRwl="WorkLis"/>
                </v-row>
...
import { GridPlugin, Toolbar, Page, Aggregate, Resize, Filter, Sort, Group, Edit, CommandColumn, Reorder, ContextMenu } from "@syncfusion/ej2-vue-grids";
...
export default {
    ...
    data(){
        return {
            ...
            PPnPersen: {locale:"id-ID", prefix: "PPn", suffix:"%", length:3, precision:0,},
            PPn: {locale:"id-ID", length:12, precision:2,},
            TotalBayar: {locale:"id-ID", prefix: "Total Bayar", suffix:"", length:12, precision:2,},
            totalPPn:"0",
            ...
            commands: [],
            commandsMobile:[{ buttonOption: { content: 'Details', cssClass: 'e-flat Details' } }],
            MataUang:[],
            Pekerjaan:[],
            Perkiraan:[],
    ...
    provide: {
        grid: [Page, Toolbar, Aggregate, Resize, Filter, Sort, Group, Edit, CommandColumn, Reorder, ContextMenu]
    },
    mounted() {
        // check actions 
        if (this.action.some(a => a == 'R')) {
        this.items_periode = JSON.parse(localStorage.getItem('Periode')).map(x => x.Nama)
        // this.autoTanggal()
        this.btn_datagrid = false
        this.tanggal_awal = JSON.parse(localStorage.getItem('Periode'))[0].TglAwal
        this.tanggal_akhir = JSON.parse(localStorage.getItem('Periode'))[0].TglAkhir
        this.getDataPurchaseOrder(JSON.parse(localStorage.getItem('Periode'))[0].TglAwal,JSON.parse(localStorage.getItem('Periode'))[0].TglAkhir)
        ...
        this.commands.push(
            { title:"Edit", buttonOption: { content: " ", cssClass: 'e-flat Edit', iconCss: 'e-edit e-icons' } },
        )
        }
        if (this.action.some(a => a == 'D')) {
        this.commands.push(
            {title:"Hapus", buttonOption: { content: " ", cssClass: 'e-flat Delete', iconCss: 'e-delete e-icons' } },
        )
        }
    ...
    created(){
        ...
        this.$route.params.contextMenu.split(',').forEach(element => {
            this.contextMenuItems.push({text:element,target: '.e-content', id: element.replace(/\s/g, '')})
        });
        ...
        if (this.$route.params.printOut != 'No Printed Document') {
            this.$route.params.printOut.split(',').forEach(element => {
                this.printOutItems.push({text:element,target: '.e-content', id: element.replace(/\s/g, '')})
            });
        }
        if (this.printOutItems.length > 0) {
            this.commands.push(
                { title:"Print", buttonOption: { cssClass: 'e-flat Print', iconCss: 'e-print e-icons' } },
            )
        }
    },
    computed: {
        formTitle(){
            return this.editedIndex === -1 ? "Tambah Purchase Order Jasa" : "Edit Purchase Order Jasa";
        },
    },
    watch: {
      windowSize(){
    ...
    methods:{
        async getDataPrint(){
            ...
            let endPoint = this.reportDesign.id.includes('Tmj') == true ? 'report-po/tmj/' : this.reportDesign.id.includes('NoDiskon') == true ? 'report-po/no-disc/' : this.reportDesign.id.includes('PoJasa') == true ? 'report-po/jasa/' : 'report-po/'
            // console.log(endPoint)
            var rute = {
                name : this.reportDesign.id.substr(1),
            }
            await api.get(endPoint+this.editedItem.id+'?token='+this.token).then(
                res => {
                // console.log(res)
        ...
        UbahSupplier(){
            this.isLoading = true
            this.editedItem.Supplier = this.supplierAll.find(s => s.Nama == this.editedItem.Supplier).Kode
            this.editedItem.BillFrom = this.supplierAll.find(s => s.Nama == this.editedItem.BillFrom).Kode
            this.editedItem.SellFrom = this.supplierAll.find(s => s.Nama == this.editedItem.SellFrom).Kode
            api.put("/purchase-orders-supplier/"+this.editedItem.id+'?token='+this.token,this.editedItem).then((res)=>{
      ...
      onResize(){
        ...
        Batalin(ket){
            api.put("/batal-po/"+this.editedItem.id+'?token='+this.token,{
                keterangan:ket
            }).then((res)=>{
                if (res.data.message == 'received') {
        ...
        TambahData(){
            this.editedItem.MataUang = this.MataUang.find(m => m.Nama == this.editedItem.MataUang).Kode
            this.editedItem.Supplier = this.supplier.find(s => s.Nama == this.editedItem.Supplier).Kode
            this.editedItem.BillFrom = this.supplier.find(s => s.Nama == this.editedItem.BillFrom).Kode
            this.editedItem.SellFrom = this.supplier.find(s => s.Nama == this.editedItem.SellFrom).Kode
            this.editedItem.items = this.itemJasa.map(j => {
                j.Perkiraan = this.Perkiraan.find(p => p.Nama == j.Perkiraan).Kode
                j.JenisPekerjaan = this.Pekerjaan.find(p => p.Nama == j.JenisPekerjaan).Kode
                return j
            })
            api.post('/purchase-orders-jasa?token='+this.token,this.editedItem)
            .then(() => {
                this.isLoading = false
                this.close()
                this.getDataPurchaseOrder(this.tanggal_awal,this.tanggal_akhir)
        ...
        UpdateData(){
            this.editedItem.hapus_items = this.hapus_items.map(f => {
                f.JenisPekerjaan = this.Pekerjaan.find(p => p.Nama == f.JenisPekerjaan).Kode
                return f
            })
            this.editedItem.items = this.itemJasa.filter(f => f.KodeNota != undefined).map(j => {
                j.Perkiraan = this.Perkiraan.find(p => p.Nama == j.Perkiraan).Kode
                j.JenisPekerjaan = this.Pekerjaan.find(p => p.Nama == j.JenisPekerjaan).Kode
                return j
            })
            this.editedItem.new_items = this.itemJasa.filter(f => f.KodeNota == undefined).map(j => {
                j.Perkiraan = this.Perkiraan.find(p => p.Nama == j.Perkiraan).Kode
                j.JenisPekerjaan = this.Pekerjaan.find(p => p.Nama == j.JenisPekerjaan).Kode
                return j
            })
            api.put('purchase-orders-jasa/'+this.editedItem.id+'?token='+this.token,this.editedItem)
            .then(() => {
                this.isLoading = false
                this.close()
                this.getDataPurchaseOrder(this.tanggal_awal,this.tanggal_akhir)
        ...
        itemsPoJasa(data){
            this.itemJasa = data
            this.editedItem.DPP = data.reduce((a,b) => a + parseFloat(b.SubTotal), 0)
            this.editedItem.diskon = data.reduce((a,b) => a + parseFloat(b.Jumlah* parseFloat(b.Harga * b.Diskon /100)), 0)
            this.editedItem.PPn = parseInt(this.editedItem.PPnPersen) * parseFloat(this.editedItem.DPP) / 100
            this.editedItem.TotalBayar = parseFloat(this.editedItem.DPP) + parseFloat(this.editedItem.PPn)
        },
        TambahPPn(){
        ...
        getDataPurchaseOrder(from,to){
            this.isLoading = true
            api.get('/purchase-orders-jasa/'+from+'/'+to+'/?token='+this.token).then(
                res => {
                    this.isLoading = false
        ...
        commandClick: function(args){
            if (args.target.classList.contains("Details")) {
              ...
              this.isLoading = true
                ...
                api.get('/purchase-orders-jasa/'+data.rowData.id+'?token='+this.token).then(
                res=>{
                    let period = JSON.parse(localStorage.getItem('Periode')).find(d => d.Kode === res.data.data.DiBuatTgl.substr(0,4)+res.data.data.DiBuatTgl.substr(5,2)) 
            ...
            if (r == true) {
                ...
                res=>{
                    ...
                    v.Diskon = v.Diskon1
                    v.SubTotal = v.Jumlah * (parseFloat(v.Harga) - (parseFloat(v.Harga) * parseFloat(v.Diskon) / 100))
                    v.rwl = res.data.rwl.find(p=> p.JenisPekerjaan == v.JenisPekerjaan)
                    v.JenisPekerjaan = v.pekerjaan.Nama
                    v.Perkiraan = v.perkiraan.Nama
                    return v
                    })
                ...
                api.put('/po-apply/' + args.rowData.id +'?token='+this.token, {
                    apply: apply
                })
                .then((res)=>{
                    if (res) {
        ...
        getDataWo(skip,take){
            api.get('purchase-orders-wo?token='+this.token+'&skip='+skip+'&take='+take).then(
                res => {
        ...
        dataStateChange: function(state) { 
            if (state.action.requestType === 'paging') {
                ...
                api.get('purchase-orders-wo?token='+this.token+'&search='+state.search[0].key+'&skip='+state.skip+'&take='+state.take).then(
                    res => {
                        this.WorkOrder = res.data
                    }
                )
                } else if (state.search == undefined) {
                    this.getDataWo(state.skip,state.take,this.sorted != null ? this.sorted : undefined,this.filter != null ? this.filter : undefined)
                }
            }
            ...
            if (state.action.requestType === "searching") {
                state.skip = 0
                state.take = 100
                if (state.action.searchString == "" && this.$refs.ggs.$el.ej2_instances[0].pageSettings.currentPage == 1) {
                    this.getDataWo(0,100)
                } else if (this.$refs.ggs.$el.ej2_instances[0].pageSettings.currentPage == 1) {
                api.get('purchase-orders-wo?token='+this.token+'&search='+state.action.searchString+'&skip='+0+'&take='+100).then(
                res => {
                    this.WorkOrder = res.data
                }
                )
        ...
        loadWorkList(){
            api.get('purchase-orders-rwl?token='+this.token+'&wo='+this.editedItem.NomorWO).then(
                res => {
                    if (res.data.dataPj === null) {
                        ...
                        res.data.data.forEach(rwl => {
                            let findIndexPj = res.data.dataPj.items_jasa.findIndex(p => p.JenisPekerjaan == rwl.JenisPekerjaan)
                            if (findIndexPj == -1) {
                                wl.push(rwl)
                            } else {
                                if (parseInt(res.data.dataPj.items_jasa[findIndexPj].Jumlah) < parseInt(rwl.Jumlah)) {
                                    rwl.Jumlah = parseInt(rwl.Jumlah) - parseInt(res.data.dataPj.items_jasa[findIndexPj].Jumlah)
                                    rwl.JumlahSisa = parseInt(rwl.Jumlah) - parseInt(res.data.dataPj.items_jasa[findIndexPj].Jumlah)
                                    wl.push(rwl)
                                }
                            }
        ...
        change_date(a){
            this.tanggal_awal = JSON.parse(localStorage.getItem('Periode')).find(d => d.Nama === a).TglAwal
            this.tanggal_akhir = JSON.parse(localStorage.getItem('Periode')).find(d => d.Nama === a).TglAkhir
        },
